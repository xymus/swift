...
public:
  explicit ModularizationError(DeclName name,
                               Kind kind,
                               Identifier expectedModuleName,
                               StringRef referencedFromModuleName,
                               Identifier foundModuleName,
                               XRefTracePath path):
    name(name), kind(kind), expectedModuleName(expectedModuleName),
    referencedFromModuleName(referencedFromModuleName),
    foundModuleName(foundModuleName), path(path) {}

  void diagnose(ASTContext &ctx,
  ...
  void log(raw_ostream &OS) const override {
    ...
    switch (kind) {
      ...
      case Kind::TypeChanged:
        OS << "type changed between what was imported from '";
...
public:
  explicit CausedError (std::unique_ptr<llvm::ErrorInfoBase> reason) :
...
public:
  explicit TypeError(DeclName name, std::unique_ptr<ErrorInfoBase> reason,
                     Flags flags={}, unsigned numVTableEntries=0)
      : CausedError(std::move(reason)) {
    this->name = name;
    this->flags = flags;
    this->numVTableEntries = numVTableEntries;
  }

  void diagnose(ASTContext &ctx) const;
...
public:
  SILEntityError(StringRef name, std::unique_ptr<ErrorInfoBase> reason)
      : CausedError(std::move(reason)), name(name) {}
